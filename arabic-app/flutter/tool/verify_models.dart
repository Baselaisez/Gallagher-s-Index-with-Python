...
void main(List<String> args) {
  ...
  for (final f in noteDir.listSync().whereType<File>().toList()
    ...
    const groups = {'sarf', 'nahw', 'awamil', 'balagha', 'bayan', 'badi'};   // badi: the registry's sixth group (wave 18)
  ...
  for (final pkg in pkgDirs..sort((a, b) => a.path.compareTo(b.path))) {
    ...
    if (morphFile.existsSync()) {
      ...
      morph.verbs.forEach((lex, v) {
        ...
        // A jamid verb stores exactly the tenses that exist — لَيْسَ is
        // mazi-only, مَا زَالَ adds a mudari; neither has an amr.
        if (v.jamid) {
          if (v.mudari.isNotEmpty && v.mudari.length != 14) {
            fail(where, 'jamid mudari has ${v.mudari.length} cells');
          }
          if (v.amr.isNotEmpty) fail(where, 'jamid verb carries amr');
          return;
        }
