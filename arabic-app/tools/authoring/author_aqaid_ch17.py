# -*- coding: utf-8 -*-
"""Author chapter 17 of aqaid-ahl-al-sunna — the best of mankind, and the order.

Continues the matn after chapter 16 closes the karamat argument: the best of mankind after our Prophet
...
TITLE17 = {"ar": "أَفْضَلُ الْبَشَرِ وَتَرْتِيبُ الْخِلَافَةِ",
...
(PKG / "chapters/17.json").write_text(json.dumps({"chapter": 17, "sentences": S}, ensure_ascii=False, indent=1), encoding="utf-8")
if not any(c["n"] == 17 for c in man["chapters"]):
    man["chapters"].append({"n": 17, "title": TITLE17})
man["chapters"].sort(key=lambda c: c["n"]); man["version"] = "0.15.0"
NOTE = ("Ch17 divergence: the supplied transcription reads «عثمان ذي النورين» with "
...
print("ch17:", len(S), "sentences,", sum(len(x["tokens"]) for x in S), "tokens; gloss +", len(GLOSS_ADD))
